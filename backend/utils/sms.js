...
const sendSMS = async (to, text) => {
  try {
    console.log('Ì≥± Twilio SMS Details:');
    console.log('   To:', to);
    ...
    if (text.includes('OTP') || text.includes('otp')) {
      ...
      if (otpMatch) {
        console.log('Ì¥ê OTP (check console for testing):', otpMatch[0]);
      }
    }
...
const sendBookingConfirmation = async (phone, date, timeSlot) => {
  console.log('Ì≥Ö Sending Booking Confirmation:', { phone, date, timeSlot });
